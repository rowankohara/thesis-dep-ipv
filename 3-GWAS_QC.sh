...
# eur has already been QC'd and has the correct file structure
# SNPS - SNP rsid, A1 - allele 1, A2 - allele 2, BETA - effect, SE - standard error, P - p value

afr <- afr %>%
  select(c(rsid, A1, A2, EFFECT, SE, P)) %>%
  rename("SNPS" = "rsid",
         "BETA" = "EFFECT")
dim(afr) # 7,410,896 SNPs

eas <- eas %>%
  select(c(MarkerName, Allele1, Allele2, Effect, StdErr, P.SE)) %>%
  rename("SNPS" = "MarkerName",
         "A1" = "Allele1",
         "A2" = "Allele2",
         "EFFECT" = "Effect",
         "SE" = "StdErr",
...
afr.r3 <- afr.r2[grep("rs", afr.r2$SNPS),]
eas.r3 <- eas.r2[grep("rs", eas.r2$SNPS),]
eur.r3 <- eur.r2[grep("rs", eur.r2$SNPS),]
...
afr.final <- afr.r3[duplicated(afr.r3$SNPS) == F,]
eas.final <- eas.r3[duplicated(eas.r3$SNPS) == F,]
eur.final <- eur.r3[duplicated(eur.r3$SNPS) == F,]
